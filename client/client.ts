...
RegisterCommand('car', async (src: number, args: string[], raw: string) => {
  console.log('hello')
  const [x, y, z] = GetEntityCoords(PlayerPedId(), true);
  const veh = await Vehicle.Create(true, args[0], x, y, z, GetEntityHeading(PlayerPedId()))
  console.log(veh)
}, false)

RegisterCommand('dv', (src: number, args: string[], raw: string) => {
...
RegisterCommand('coords', (src: number, args: string[], raw: string) => {
